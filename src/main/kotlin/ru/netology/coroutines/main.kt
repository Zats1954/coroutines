...
private val postComments: Map<Int, Comment> = HashMap()
...
fun main() {
    with(CoroutineScope(EmptyCoroutineContext)) {
        launch {
            try {
                getPosts(client).map { post ->
                        async {
                            PostWithComments(post, getComments(client, post.id)).let {
                                postComments.plus(Pair(post.id,it))
                                postPrint(it)
                            }
                        }
                    }.awaitAll()
...
@Synchronized  fun postPrint(postWithComments: PostWithComments) {
    println("post  ${postWithComments.post.author}")
    println(postWithComments.post.content)
